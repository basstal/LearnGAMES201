...
extern std::wstring wProjectName;
...
void getProjectFilePath(const char *projectRelativePath2File, std::string &outputPath)
{
    ...
    if (!inPath.empty() && inPath.find(projectName) == -1)
    {
        wchar_t absolutePath[300];
        _wgetcwd(absolutePath, 300);
        std::wstring pathStr(absolutePath);
        std::wstring::size_type index = pathStr.rfind(wProjectName);
        pathStr = pathStr.substr(0, index + wProjectName.length());
        outputPath.clear();
