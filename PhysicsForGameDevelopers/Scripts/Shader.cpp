...
void Shader::readAndCompileAll(const GLchar * vertexPath, const GLchar * fragmentPath, const GLchar * geometryPath)
{
	...
	if(vertexPath && vertexPath[0] != '\0' && std::string(vertexPath).find(projectName) == -1)
	{
	...
	if (fragmentPath && fragmentPath[0] != '\0' && std::string(fragmentPath).find(projectName) == -1)
	{
	...
	if (geometryPath && geometryPath[0] != '\0' && std::string(geometryPath).find(projectName) == -1)
	{
