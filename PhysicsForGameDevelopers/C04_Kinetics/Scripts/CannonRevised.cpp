...
float m = 100, Cd = 30.0f, Vw = 10.0f, GammaW = 90.0f, Cw = 10.0f;
float g = 0.98f;
...
void draw_imgui(GLFWwindow *window)
{
    ...
    ImGui::InputFloat("drag coefficient", &Cd);
    ImGui::InputFloat("wind gamma", &GammaW);
    ImGui::InputFloat("wind speed", &Vw);
    ImGui::InputFloat("wind drag coefficient", &Cw);
}
...
void simulate(float delta)
{
    ...
    if ((simulation_count < simulation_steps && !bSimulatePaused) || bNextStep)
    {
        ...
        sy1 = yStart + cannonLength * cosAlpha;
        vy1 = vm * cosThetaY;

        sz1 = ze;
        vz1 = vm * cosThetaZ;

        s.x = ((m / Cd) * exp(-(Cd * t) / m) * ((-Cw * Vw * cos(glm::radians(GammaW))) / Cd - vx1) - (Cw * Vw * cos(glm::radians(GammaW)) * t) / Cd) - ((m / Cd) * ((-Cw * Vw * cos(glm::radians(GammaW))) / Cd - vx1)) + sx1;
        s.y = sy1 + (-(vy1 + (m * g) / Cd) * (m / Cd) * exp(-(Cd * t) / m) - (m * g * t) / Cd) + ((m / Cd) * (vy1 + (m * g) / Cd));
        s.z = ((m / Cd) * exp(-(Cd * t) / m) * ((-Cw * Vw * sin(glm::radians(GammaW))) / Cd - vz1) - ((Cw * Vw * sin(glm::radians(GammaW)) * t) / Cd)) - ((m / Cd) * ((-Cw * Vw * sin(glm::radians(GammaW))) / Cd - vz1)) + sz1;
        // v = glm::vec3(
        //     vm * cosThetaX,
        //     (vm * cosThetaY) - g * t,
        //     vm * cosThetaZ);
