...
num_particles = ti.field(ti.i32, ())
spring_stiffness = ti.field(ti.f32, ())
paused = ti.field(ti.i32, ())
damping = ti.field(ti.f32, ())

particle_mass = 1
bottom_y = 0.05

x = ti.Vector.field(2, float, max_num_particles)
v = ti.Vector.field(2, float, max_num_particles)

A = ti.Matrix.field(2, 2, float, (max_num_particles, max_num_particles))
b = ti.Vector.field(2, float, max_num_particles)

# rest_length[i, j] = 0 means i and j are not connected
rest_length = ti.field(ti.f32, (max_num_particles, max_num_particles))
